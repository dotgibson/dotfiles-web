...
// `npm run coverage` from a worktree finds no dotfiles-Defense and — on the lenient
// path — exits 0 keeping the stale snapshot. `git rev-parse --git-common-dir` resolves
// to the MAIN worktree's .git even from a linked one, so its grandparent is the fleet
// directory; the `!== beside` guard makes step 3 a no-op in a normal clone.
//
// The sentinel is THIS script's own source (dotfiles-Defense), not dotfiles-core: a
// direct test of "is my input here?" rather than a proxy for it.
...
const { root, via: rootVia } = resolveFleetRoot('dotfiles-Defense');
if (rootVia === 'main worktree') {
  ...
  console.log(`[collect-coverage] running from a linked worktree — reading the fleet at ${root}`);
}
